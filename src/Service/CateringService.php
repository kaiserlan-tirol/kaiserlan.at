...
{
    private const FOODFLAT_ADDON_NAME = 'Foodflat';
    ...
    /**
     * Statistics of all ordered products, split into items covered by a flatrate and paid items.
     *
     * @return array [['product' => CateringProduct, 'flat' => int, 'paid' => int, 'revenue' => int, 'inFlat' => bool],...]
     */
    public function getProductStatistics(): array
    {
        $stats = $this->orderPositionRepository->getProductStatistics(CateringOrderStatus::STATUS_NOT_DEAD);

        $result = [];
        foreach ($this->productRepository->findAll() as $product) {
            if (!isset($stats[$product->getId()])) {
                continue;
            }
            $result[] = [
                'product' => $product,
                'flat' => $stats[$product->getId()]['flat'],
                'paid' => $stats[$product->getId()]['paid'],
                'revenue' => $stats[$product->getId()]['revenue'],
                'inFlat' => !$product->getIncludedInAddons()->isEmpty(),
            ];
        }

        usort($result, fn($a, $b) => [$a['product']->getSortIndex() ?? PHP_INT_MAX, $a['product']->getName()]
            <=> [$b['product']->getSortIndex() ?? PHP_INT_MAX, $b['product']->getName()]);

        return $result;
    }

    /**
     * Revenue generated by the foodflat addon, calculated as the number of ordered addons times its price.
     *
     * @return array ['addon' => ShopAddon|null, 'count' => int, 'revenue' => int]
     */
    public function getFoodflatRevenue(): array
    {
        $addon = null;
        // search all addons, the foodflat is usually deactivated once the event is over
        foreach ($this->shopService->getAddons(all: true) as $item) {
            if ($item->getName() === self::FOODFLAT_ADDON_NAME) {
                $addon = $item;
                break;
            }
        }
        ...
        if (is_null($addon)) {
            return ['addon' => null, 'count' => 0, 'revenue' => 0];
        }

        $count = $this->shopService->countOrderedAddon($addon);
        return ['addon' => $addon, 'count' => $count, 'revenue' => $count * $addon->getPrice()];
    }
